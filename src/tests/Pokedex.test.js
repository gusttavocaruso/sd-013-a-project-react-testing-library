...
describe('1 - Teste o componente <Pokedex.js />.', () => {
  it('A página contém um heading h2 com o texto Encountered pokémons', () => {
    renderWithRouter(<Pokedex pokemons={ pokemons } isPokemonFavoriteById={ favoritePokemons } />);
    const h2 = screen.queryByText(/Encountered pokémons/i);
    expect(h2).toBeInTheDocument();
    expect(h2).toContainHTML('</h2>');
  });
});
describe('2 - Teste se é exibido o próximo Pokémon da lista quando o botão Próximo pokémon é clicado', () => {
  ...
  it('2.2 - Os próximos Pokémons da lista devem ser mostrados, um a um, ao clicar sucessivamente no botão', () => {
    renderWithRouter(<Pokedex pokemons={ pokemons } isPokemonFavoriteById={ favoritePokemons } />);
  ...
  it('2.3 - O primeiro Pokémon da lista deve ser mostrado ao clicar no botão, se estiver no último Pokémon da lista', () => {
    renderWithRouter(<Pokedex pokemons={ pokemons } isPokemonFavoriteById={ favoritePokemons } />);
...
describe('3 - Teste se a Pokédex tem os botões de filtro', () => {
  it('3.1 - Deve existir um botão de filtragem para cada tipo de Pokémon, sem repetição.', () => {
    renderWithRouter(<Pokedex pokemons={ pokemons } isPokemonFavoriteById={ favoritePokemons } />);
  ...
  it('3.2 - A partir da seleção de um botão de tipo, a Pokédex deve circular somente pelos pokémons daquele tipo', () => {
    renderWithRouter(<Pokedex pokemons={ pokemons } isPokemonFavoriteById={ favoritePokemons } />);
    ...
    pokemonsFilteredByType.forEach(({ name }) => {
      const pokemon = screen.queryByTestId(pokemonName);
      expect(pokemon).toHaveTextContent(name);
      console.log(name);
      fireEvent.click(screen.queryByRole('button', { name: /Próximo pokémon/i }));
